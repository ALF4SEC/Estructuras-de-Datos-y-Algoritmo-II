...
int main(){
	...
	strcpy(reg.nombre, "Alfonso");
	strcpy(reg.ape1 ,"Crego");
	strcpy(reg.ape2, "Calvo");
	strcpy(reg.provincia, "Salamanca");

	i=insertarReg(f,&reg);

	if (i<0){ 
		printf("Registro insertado en el area de desborde");
		printf("\t %s %s %s %s %s\n",reg.dni,reg.nombre,reg.ape1,reg.ape2,reg.provincia);
	} else { 
		printf("Registro insertado en el CUBO %d",i);
		printf("\t %s %s %s %s %s\n",reg.dni,reg.nombre,reg.ape1,reg.ape2,reg.provincia);
	}
